...
import com.recomo.common.preview.urdf.UrdfCache
...
class FilamentPreviewRenderer(private val context: Context) {
    ...
    private var previewModel: com.recomo.common.preview.urdf.UrdfModel? = null
    ...
    fun hitTestAt(screenX: Float, screenY: Float, viewWidth: Int, viewHeight: Int): String? {
        val ground = screenToGroundPlane(screenX, screenY, viewWidth, viewHeight) ?: return null
        val (gx, gy) = ground
        // Threshold scales with orbit distance for consistent tap feel
        val threshold = (orbitDistance * 0.15).coerceIn(0.3, 2.0)
        val robotDist = kotlin.math.hypot(gx - robotWorldX, gy - robotWorldY)
        val avatarDist = if (avatarAsset != null && avatarVisible)
            kotlin.math.hypot(gx - avatarWorldX, gy - avatarWorldY)
        else Double.MAX_VALUE
        val minDist = minOf(robotDist, avatarDist)
        if (minDist > threshold) return null
        return if (robotDist <= avatarDist) "robot" else "avatar"
    }
