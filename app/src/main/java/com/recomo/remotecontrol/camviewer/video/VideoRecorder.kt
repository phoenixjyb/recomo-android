    ...
    suspend fun startRecording(codec: String = "h265"): String? = withContext(Dispatchers.IO) {
        mutex.withLock {
            ...
            try {
                ...
                // Use app-scoped external storage (works on Android 11+ without special permissions)
                val moviesDir = context.getExternalFilesDir(Environment.DIRECTORY_MOVIES)
                val outputDir = File(moviesDir, "recomoVideosRawStream").apply {
                    if (!exists()) mkdirs()
                }
        ...
        mutex.withLock {
            ...
            try {
                ...
                if (muxerStarted && trackIndex >= 0) {
                    ...
                    // Note: MediaMuxer on Android accepts Annex-B format (with start codes).
                    // Stripping start codes can corrupt multi-NAL frames (VPS+SPS+PPS+IDR).
                    // Pass the raw Annex-B data directly — MediaMuxer handles it correctly.
