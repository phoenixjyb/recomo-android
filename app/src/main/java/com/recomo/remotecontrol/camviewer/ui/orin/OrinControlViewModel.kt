package com.recomo.remotecontrol.camviewer.ui.orin
...
class OrinControlViewModel @Inject constructor(
    ...
    suspend fun requiresPin(): Boolean {
        ...
        return settings.serviceControlPinEnabled && settings.serviceControlPin.isNotBlank()
    }
    ...
    fun startServices(enteredPin: String? = null) {
        viewModelScope.launch {
            ...
            val settings = settingsRepository.settings.first()
            if (settings.serviceControlPinEnabled && settings.serviceControlPin.isNotBlank()) {
