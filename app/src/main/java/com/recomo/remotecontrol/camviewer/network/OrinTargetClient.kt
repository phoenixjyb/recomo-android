...
import com.recomo.remotecontrol.camviewer.di.KtorHttpClient
...
class OrinTargetClient @Inject constructor(
     ...
     * @return Result containing trackingId if available
     */
    ...
    suspend fun sendTargetROI(
        ...
        return try {
            ...
            if (response.status.isSuccess()) {
                val body = runCatching { response.body<TargetResponse>() }.getOrNull()
                Log.i(TAG, "Target sent successfully: ${response.status}, trackingId=${body?.trackingId}")
                Result.success(body?.trackingId)
            } else {
    ...
    /**
     * Tracking lifecycle request (start/stop/reset).
     */
    @Serializable
    data class TrackingCycleRequest(
        val action: String,
        @SerialName("tracking_id") val trackingId: String? = null,
    ...
    /**
     * Tracking lifecycle response.
     */
    @Serializable
    data class TrackingCycleResponse(
        val status: String,
        val action: String,
        @SerialName("tracking_id") val trackingId: String? = null,
    ...
    /**
     * Send tracking lifecycle signal (start/stop/reset).
     */
    suspend fun sendTrackingCycle(
        baseUrl: String,
        action: String,
        ...
        return try {
            ...
            val targetUrl = "$url/tracking/cycle"

            val response: HttpResponse = httpClient.post(targetUrl) {
                contentType(ContentType.Application.Json)
                setBody(
                    TrackingCycleRequest(
                        action = action,
            ...
            if (response.status.isSuccess()) {
                val body = runCatching { response.body<TrackingCycleResponse>() }.getOrNull()
                Log.i(TAG, "Tracking cycle $action sent: ${response.status}, trackingId=${body?.trackingId}")
                Result.success(body?.trackingId)
            } else {
                val error = "Failed to send tracking cycle: ${response.status}"
                Log.e(TAG, error)
                Result.failure(Exception(error))
            }
        } catch (e: Exception) {
            Log.e(TAG, "Error sending tracking cycle", e)
            Result.failure(e)
        }
    }
}
